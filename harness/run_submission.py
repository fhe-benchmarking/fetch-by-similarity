...
def main():
    ...
    cmd = ["python3", exec_dir/"client_key_generation.py", str(size)]
    if args.count_only:
    ...
    subprocess.run(["python3", exec_dir/"server_preprocess_dataset.py", str(size)], check=True)
    ...
    for run in range(args.num_runs):
        ...
        cmd = ["python3", exec_dir/"server_encrypted_compute.py", str(size)]
        if args.count_only:
            cmd.extend(["--count_only"])
        subprocess.run(cmd, check=True)
        ...
        cmd = ["python3", exec_dir/"client_postprocess.py", str(size)]
        if args.count_only:
            cmd.extend(["--count_only"])
        subprocess.run(cmd, check=True)
